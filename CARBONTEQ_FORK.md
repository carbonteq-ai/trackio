...
`0.31.5.post14.dev22` is permanently unusable: its distribution metadata was
dev22 while `trackio.__version__` remained dev21. The development-index
clean-install gate detected the skew, and no framework lock may consume it.
...
`0.31.5.post14.dev12` executes a validated batch in one storage transaction
and resolves the page's trace keys in one bounded query. It retains the same
...
`0.31.5.post14.dev15` writes fresh scalar projections with one bounded Doris
